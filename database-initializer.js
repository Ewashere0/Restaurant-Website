let userNames = ["winnifred", "lorene", "cyril", "vella", "erich", "pedro", "madaline", "leoma", "merrill",  "jacquie", "ethan"];
let users = [];
...
users[users.length - 1].admin = true;
		...
		db.collection(collection.name).drop(function(err, delOK){
			...
			if(numDropped == toDrop){
				db.collection("users").insertMany(users, function(err, result){
					...
					console.log(result.insertedCount + " users successfully added (should be 11).");
					client.close();
				});
			}
		});		
